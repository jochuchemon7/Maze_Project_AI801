...
# https://en.wikipedia.org/wiki/Maze_generation_algorithm  (PRIM'S ALGORITHM)
...
for i in range(sample_maze.shape[0]):
    for j in range(sample_maze.shape[1]):
        visited[(i, j)] = False

def dfs(maze, visited, node, target_node):
    ...
    for adjacent_x, adjacent_y in [(-1, 0), (0, 1), (1, 0), (0, -1)]:  # Top, right, bottom and left neighbors
        ...
        if 0 <= new_node[0] < len(maze) and 0 <= new_node[1] < len(maze[0]) and not visited[(new_node[0], new_node[1])] and maze[new_node[0], new_node[1]] != wall:
            if dfs(maze, visited, new_node, target_node):
                return True, visited
    return False
...
demo_found, demo_result = dfs(env.maze, visited, start_node, target_node)

demo_solution = np.zeros((sample_maze.shape[0], sample_maze.shape[1]))
for i in range(sample_maze.shape[0]):
    for j in range(sample_maze.shape[1]):
        print([i, j], ' demo_result:  ', demo_result[(i, j)])
        if demo_result[(i, j)]:
            demo_solution[i, j] = 1
plt.imshow(demo_solution)

raw_maze = env.get_maze_()
